...
def parse_file( fname, edges, polygons, csystems, screen, color ):
    ...
    while c < len(lines):
        ...
        if line == 'sphere':
            ...
            add_sphere(polygons, # 1) Add the shape to a temporary matrix
            float(args[0]), float(args[1]), float(args[2]),
            float(args[3]), step_3d)
            matrix_mult( csystems[len(csystems)-1], polygons )  #2) Multiply that matrix by the current top of the coordinate system stack
            draw_polygons(polygons, screen, color) # 3) Draw the shape to the screen
            polygons = []   #4) Clear the temporary matrix
            ...
            csystems[len(csystems)-1] = [ x[:] for x in t]
